...
示例：
    python start.py ingest --pdf EOS.pdf --kb-id demo --to-milvus
    python start.py ingest --pdf EOS.pdf --kb-id demo --to-milvus --recreate
    python start.py query "编译环境怎么配置" --kb-id demo
    python start.py query                       # 进入交互式多轮问答
    python start.py recreate
"""
...
from utils import embedding, milvus, sqlite_store
...
def _run_ingest(args: argparse.Namespace) -> None:
    ...
    # 3.5 (可选) 保存文档级 + chunk 级元数据到 SQLite（覆盖式）
    if args.to_sqlite:
...
def _add_ingest_parser(subparsers) -> None:
    ...
    p.add_argument("--pdf", default=str(PROJECT_DIR / "EOS.pdf"))
    p.add_argument("--out", default=str(PROJECT_DIR / "mineru_output"))
    ...
    p.add_argument(
        "--to-sqlite",
        action="store_true",
        help="把文档级与 chunk 级元数据保存到 SQLite/rag_metadata.db（同 document_id 覆盖）",
    )
    p.add_argument(
        "--recreate",
        action="store_true",
